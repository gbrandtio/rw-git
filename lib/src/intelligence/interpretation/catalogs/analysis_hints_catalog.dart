  ...
  AnalysisType.bugHotspots: ToolHints(
    interpretation: [
      'Defects in a codebase are not spread evenly: a consistent, '
          ...
          'defects (Ostrand et al. 2004) — which means the right way to use '
          'this tool\'s output is to focus attention on the top slice of '
          'flagged files, not to treat every file with a nonzero bug count '
          'as equally concerning. A file\'s history of past defects is '
          'itself one of the strongest available predictors of where future '
          'defects will appear, often outperforming any single structural '
  ...
  AnalysisType.codeVolatility: ToolHints(
    interpretation: [
      'Two independent, separately validated signals combine in this '
          ...
          'changes is a better defect predictor than how large the file is, '
          'and the relationship is not proportional — a file that changes '
          'twice as often carries more than twice the expected defect risk, '
          'not exactly twice (Nagappan & Ball 2005; Ostrand et al. 2004). '
          'Separately, how many different people have touched a file '
          'improves defect prediction beyond churn alone — a file many '
          'authors have to coordinate around carries more risk than the '
          'same amount of churn from one person working alone, the '
          '"too many cooks" effect (Weyuker, Ostrand & Bell 2008), which is '
          'also a natural consequence of Conway\'s Law coordination '
          'overhead (Conway 1968). A file ranking high on this tool\'s '
          'output is one that is both changing frequently and being worked '
          'on by many people at once — the combination the literature '
          'treats as the most concerning, not either factor alone.',
    ],
    pairWith: [
  ...
  AnalysisType.universalLexicalMetrics: ToolHints(
    interpretation: [
      'Several independent, well-studied complexity measures are reported '
          ...
          'function with a cyclomatic complexity of ten or below is '
          'realistically testable with a manageable number of test cases; '
          'eleven to twenty needs deliberate review before more logic is '
          'added; above twenty, exhaustively testing every path becomes '
          'practically impossible (McCabe 1976). A maintainability index '
          'of 85 or above describes highly maintainable code; 65 to 84 is '
          'moderate; below 65 is a genuine signal that the code needs '
          'refactoring (Coleman et al. 1994; van Deursen 2014). An ABC '
          'score above 15 is elevated and above 30 warrants refactoring — '
          'this measure catches size and assignment-heavy complexity that '
          'cyclomatic complexity alone can miss entirely, such as a long '
          'run of straight-line assignments with no branching at all '
          '(Fitzpatrick 1997). An NPath figure above 200 means the number '
          'of distinct paths through the function has grown large enough '
          'that no team could realistically write a test for every one of '
          'them (Nejmeh 1988). Halstead\'s delivered-bugs estimate, derived '
          'from the function\'s vocabulary and length, is validated against '
          'real industrial defect data and is the single most directly '
          'useful number the Halstead family of metrics produces (Halstead '
          '1977).',
    ],
    caveats: [
