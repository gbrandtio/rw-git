...
void main() {
  group('Interceptor chain ordering', () {
    test('runs interceptors outermost-first around the terminal handler',
        () async {
      final log = <String>[];
      final url = Uri.parse('https://example.com/foo');
      final client = MockHttpClient(interceptors: [
    ...
    test('retry interceptor composes with a logging-style interceptor',
        () async {
      final log = <String>[];
      final url = Uri.parse('https://example.com/foo');
      final client = MockHttpClient(interceptors: [
