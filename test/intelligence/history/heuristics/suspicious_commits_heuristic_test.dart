...
void main() {
  group('SuspiciousCommitsHeuristic', () {
    ...
    test('findSuspiciousCommits identifies suspicious code diffs', () async {
      ...
      final heuristic = SuspiciousCommitsHeuristic(runner);
      final flagged = await heuristic.findSuspiciousCommits(
        '/test',
        limit: '10',
      );
      expect(flagged.length, 1);
    ...
    test(
      'findSuspiciousCommits handles diffs with incomplete header',
      () async {
        final runner = _MockRunner(
          streamLines: [
...
''';
    ...
    test(
      'extractChangedComments parses comments with missing header info',
      () async {
...
''';
        final runner = _MockRunner(runOutput: runOutput);
        final heuristic = SuspiciousCommitsHeuristic(runner);
        final comments = await heuristic.extractChangedComments(
          '/test',
          limit: '10',
        );
        expect(comments.length, 1);
