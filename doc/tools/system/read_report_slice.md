...
Answers: "How do I get one piece of an offloaded report back without re-inflating my context?" When a tool's output exceeds the offload threshold, the full JSON is written to `.rw_git/reports/` and only a summary plus `preview` is returned inline ([ADR-0001](../../adr/0001-file-offloading-of-large-tool-outputs.md)). A naive "read the file" step would defeat the purpose of offloading by pulling the entire payload back into context. This tool lets a tool-only model page through a big report cheaply — one key path, one page of an array at a time ([ADR-0006](../../adr/0006-targeted-retrieval-of-offloaded-reports.md)).
...
None — this is a retrieval mechanism, not an analysis. Its design follows the token-economy rationale of the offloading architecture ([ADR-0001](../../adr/0001-file-offloading-of-large-tool-outputs.md), [ADR-0006](../../adr/0006-targeted-retrieval-of-offloaded-reports.md)): models retrieve exactly the slice they need, preserving the context-window savings that offloading exists to provide.
