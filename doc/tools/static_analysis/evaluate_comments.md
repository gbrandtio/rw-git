...
This tool replaces the former `evaluate_comment_quality`, `evaluate_comment_necessity`, and `evaluate_comment_llm_generation` tools. They were merged into one tool with an `aspects` parameter to shrink the tool-selection surface a small model must reason over (see [ADR-0007](../../adr/0007-tools-list-token-budget-and-pagination.md)).
