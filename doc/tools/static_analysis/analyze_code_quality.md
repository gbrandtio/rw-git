...
Answers: "What SOLID principle violations are accumulating in this module?".Runs four complementary heuristics over commit history to surface:
- Suspicious code patterns
- Scope-creep commits
- Unstable interfaces
- Co-change clusters (blast radius)
- High-churn methods. 
Intended for module-level technical debt reviews.

With `includeAuthors: true`, it additionally answers: "Which developers own the violations?" adding per-author attribution to every churn metric. This enables targeted mentoring, knowledge-transfer planning, and evidence-based ownership decisions. The intention is not being punitive, because the individuals deciding on the violations might have made perfectly valid trade-offs.
...
Mega commits are inherently hard to review. Reviewers cannot hold the full context of 500+ lines in working memory. Mega commits also frequently bundle unrelated changes, making rollback difficult.
...
**Method churn:** Parse diff hunks at the function signature level and detect repeated modification to the same function body across commits. High method-level churn = Open-Closed Principle violation.
...
**How rw-git uses it:** The per-author churn breakdown surfaces the "too many cooks" signal at the file level. Engineering leaders can identify files where many authors each contribute small amounts which the pattern most associated with elevated defect density in Weyuker et al.'s study.
