...
**How rw-git uses it:** Dependency freshness is the complementary concern to pinning: pinned but severely outdated dependencies are also a risk. The opt-in `check_freshness` pass described above directly operationalizes this finding — it compares each declared version against the latest available release per ecosystem and surfaces how far behind (patch/minor/major) each dependency is.
