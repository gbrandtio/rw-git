...
Answers: "Which files are secretly coupled even though there is no explicit import between them?" Files that always change together reveal hidden design dependencies — the *Shotgun Surgery* code smell at the repository level. Informs refactoring opportunities, blast radius estimation, and identification of modules that violate the Single Responsibility Principle.
...
**Key claim:** Logical coupling is one of the strongest standalone bug predictors — in some projects, stronger than structural metrics like complexity or LOC. Files with many logical couplings accumulate more defects because cross-file coupling creates hidden channels for defect propagation.
